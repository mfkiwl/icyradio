...
reg  [63:0] s_axi_full_wdata = 32'd0;
reg   [7:0] s_axi_full_wstrb = 4'd0;
...
always @(posedge aclk)
    begin
        ...
        if(sim_periods == 7500)
            begin
                axi_full_read(32'h00000000, 1, 3); // Read 2 words, aligned (addr, nbeats - 1, nbytes - 1)
            end

        if(sim_periods == 7800)
            begin
                axi_full_read(32'h00000010, 1, 3); // Read 4 words, aligned, continuous address (addr, nbeats - 1, nbytes - 1)
            end
