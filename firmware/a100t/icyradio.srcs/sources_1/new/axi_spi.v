...
assign spi_io_i[2] = 1'b0;
assign spi_io_i[3] = 1'b0;
                ...
                if(s_axi_rd_en)
                    begin
                        ...
                        case(s_axi_araddr[S_AXI_ASZ - 1:S_AXI_ADDR_LSB])
                            ...
                            4'h1:    s_axi_rdata <= {2'd0, spi_io_mode_q, 4'd0, spi_fsm_state, spi_dir, spi_idle, 2'd0, spi_sr_in_buf_valid, spi_sr_out_buf_valid, 7'd0, spi_lsb_first, 2'd0, spi_io_mode, spi_mode, spi_en, spi_sck_div_en};
                            4'h2:    s_axi_rdata <= {{(S_AXI_DSZ - NUM_IRQS){1'b0}}, irq_enabled}; // IRQ Enabled
                            4'h3:    s_axi_rdata <= {{(S_AXI_DSZ - NUM_IRQS){1'b0}}, irq_pend}; // IRQ Pending
                            4'h4:    s_axi_rdata <= {{(S_AXI_DSZ / 2 - SCK_DIV_SZ){1'b0}}, spi_sck_div_h, {(S_AXI_DSZ / 2 - SCK_DIV_SZ){1'b0}}, spi_sck_div_l};
                            4'h5:    s_axi_rdata <= {22'd0, spi_rd_req, spi_sr_out_buf_valid, spi_sr_out_buf};
                            4'h6:    {spi_sr_in_buf_ready, s_axi_rdata} <= {spi_sr_in_buf_valid, spi_sr_in_buf_valid, 23'd0, spi_sr_in_buf};
                            4'h7:    s_axi_rdata <= {24'd0, spi_sr};
                            4'h8:    s_axi_rdata <= {{(S_AXI_DSZ - NUM_SLAVES){1'b0}}, spi_ss_o};
                        ...
                        case(s_axi_awaddr[S_AXI_ASZ - 1:S_AXI_ADDR_LSB])
                            ...
                            4'h1: // Register 1
                                begin
                                    ...
                                    //     begin
                                    //         // s_axi_wdata[29:28] is spi_io_mode_q
                                    //     end
                                end
                            4'h2: // Register 2
                                begin
                                    ...
                                    if(!spi_sck_div_en) // Clock divider can only be changed if the clock generation is disabled
                                        begin
                                            if(s_axi_wstrb[0]) // s_axi_wdata[7:0]
                                                begin
                                                    if(SCK_DIV_SZ > 8)
                                                        spi_sck_div_l[7:0] <= s_axi_wdata[7:0];
                                                    else
                                                        spi_sck_div_l[SCK_DIV_SZ - 1:0] <= s_axi_wdata[SCK_DIV_SZ - 1:0];
                                                end

                                            if(SCK_DIV_SZ > 8 && s_axi_wstrb[1]) // s_axi_wdata[15:8]
                                                begin
                                                    spi_sck_div_l[SCK_DIV_SZ - 1:8] <= s_axi_wdata[SCK_DIV_SZ - 1:8];
                                                end

                                            if(s_axi_wstrb[2]) // s_axi_wdata[23:16]
                                                begin
                                                    if(SCK_DIV_SZ > 8)
                                                        spi_sck_div_h[7:0] <= s_axi_wdata[23:16];
                                                    else
                                                        spi_sck_div_h[SCK_DIV_SZ - 1:0] <= s_axi_wdata[SCK_DIV_SZ + 15:16];
                                                end

                                            if(SCK_DIV_SZ > 8 && s_axi_wstrb[3]) // s_axi_wdata[31:24]
                                                begin
                                                    spi_sck_div_h[SCK_DIV_SZ - 1:8] <= s_axi_wdata[SCK_DIV_SZ + 15:24];
                                                end
                                        end
                                end
                            4'h5: // Register 5
