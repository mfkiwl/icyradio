#ifndef __IDT8V97003_H__
#define __IDT8V97003_H__
...
typedef struct idt8v97003_vco_band_t idt8v97003_vco_band_t;
typedef struct idt8v97003_ref_path_config_t idt8v97003_ref_path_config_t;
typedef struct idt8v97003_charge_pump_config_t idt8v97003_charge_pump_config_t;
typedef struct idt8v97003_loop_filter_t idt8v97003_loop_filter_t;
typedef struct idt8v97003_loop_freq_response_t idt8v97003_loop_freq_response_t;

struct idt8v97003_vco_band_t
{
    uint8_t ubVCO;
    uint8_t ubDigitalBand;
};
struct idt8v97003_ref_path_config_t
{
    uint8_t ubEnableDoubler;
    uint8_t ubMult;
    uint16_t usRDiv;
};
struct idt8v97003_charge_pump_config_t
{
    float fPositiveCurrent;
    float fNegativeCurrent;
    float fBleederCurrent;
};
struct idt8v97003_loop_filter_t
{
    float fRs; // Series resistor (2nd order)
    float fCs; // Series capacitor (2nd order)
    float fCp; // Shunt capacitor (2nd order)
    float fR3; // Series resistor (3nd order)
    float fC3; // Series capacitor (3nd order)
};
struct idt8v97003_loop_freq_response_t
{
    float fFz; // Zero frequency
    float fFc; // Loop bandwidth
    float fFp; // Pole frequency
    float fFp2; // 2nd pole frequency (for 3rd order filters)
    float fPhaseMargin; // Phase margin
};

#define IDT8V97003_PWR_REF_VREG         BIT(0)
#define IDT8V97003_PWR_PDCP_VREG        BIT(1)
#define IDT8V97003_PWR_FB_VREG          BIT(2)
#define IDT8V97003_PWR_OUTA_VREG        BIT(3)
#define IDT8V97003_PWR_OUTB_BUF_VREG    BIT(4)
#define IDT8V97003_PWR_ANA_VREG         BIT(5)
#define IDT8V97003_PWR_VCO              BIT(6)
#define IDT8V97003_PWR_CHIP_ENABLE      BIT(7)
#define IDT8V97003_PWR_ALL_CE           0x7F
#define IDT8V97003_PWR_ALL              0xFF
...
uint8_t idt8v97003_is_muted(int8_t bChannel); // < 0 = is muted via MUTE pin?
