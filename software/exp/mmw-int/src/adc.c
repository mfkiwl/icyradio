...
float adc_get_rx_pwr_vdet()
{
    ...
    ADC0->SINGLECTRL = ADC_SINGLECTRL_AT_256CYCLES | ADC_SINGLECTRL_NEGSEL_VSS | ADC_RX_PWR_VDET_CHAN | ADC_SINGLECTRL_REF_5V | ADC_SINGLECTRL_RES_OVS;
    ADC0->SINGLECTRLX = ADC_SINGLECTRLX_FIFOOFACT_OVERWRITE | (0 << _ADC_SINGLECTRLX_DVL_SHIFT);
    ADC0->CAL &= ~(_ADC_CAL_SINGLEGAIN_MASK | _ADC_CAL_SINGLEOFFSET_MASK | _ADC_CAL_SINGLEOFFSETINV_MASK);
    ADC0->CAL |= (DEVINFO->ADC0CAL1 & 0x7FFF0000) >> 16; // Calibration for 5V reference
    ADC0->BIASPROG = (ADC0->BIASPROG & (_ADC_BIASPROG_VFAULTCLR_MASK | _ADC_BIASPROG_ADCBIASPROG_MASK)) | ADC_BIASPROG_GPBIASACC_HIGHACC;
    ...
    float fRXVdet = ADC0->SINGLEDATA * 5000.f / 65535.f;

    return fRXVdet * ADC_RX_PWR_VDET_DIV;
}
float adc_get_tx_pwr_vdet()
{
    while(ADC0->STATUS & (ADC_STATUS_WARM | ADC_STATUS_SCANREFWARM | ADC_STATUS_SINGLEREFWARM)); // Wait for ADC to cool down

    ADC0->SINGLECTRL = ADC_SINGLECTRL_AT_256CYCLES | ADC_SINGLECTRL_NEGSEL_VSS | ADC_TX_PWR_VDET_CHAN | ADC_SINGLECTRL_REF_2V5 | ADC_SINGLECTRL_RES_OVS;
    ADC0->SINGLECTRLX = ADC_SINGLECTRLX_FIFOOFACT_OVERWRITE | (0 << _ADC_SINGLECTRLX_DVL_SHIFT);
    ADC0->CAL &= ~(_ADC_CAL_SINGLEGAIN_MASK | _ADC_CAL_SINGLEOFFSET_MASK | _ADC_CAL_SINGLEOFFSETINV_MASK);
    ADC0->CAL |= (DEVINFO->ADC0CAL0 & 0x7FFF0000) >> 16; // Calibration for 2V5 reference
    ADC0->BIASPROG = (ADC0->BIASPROG & (_ADC_BIASPROG_VFAULTCLR_MASK | _ADC_BIASPROG_ADCBIASPROG_MASK)) | ADC_BIASPROG_GPBIASACC_HIGHACC;
    ...
    float fTXVDet = ADC0->SINGLEDATA * 2500.f / 65535.f;

    return fTXVDet * ADC_TX_PWR_VDET_DIV;
}
float adc_get_rx_lo_vpwr()
{
    while(ADC0->STATUS & (ADC_STATUS_WARM | ADC_STATUS_SCANREFWARM | ADC_STATUS_SINGLEREFWARM)); // Wait for ADC to cool down

    ADC0->SINGLECTRL = ADC_SINGLECTRL_AT_256CYCLES | ADC_SINGLECTRL_NEGSEL_VSS | ADC_RX_LO_VPWR_CHAN | ADC_SINGLECTRL_REF_5V | ADC_SINGLECTRL_RES_OVS;
    ADC0->SINGLECTRLX = ADC_SINGLECTRLX_FIFOOFACT_OVERWRITE | (0 << _ADC_SINGLECTRLX_DVL_SHIFT);
    ADC0->CAL &= ~(_ADC_CAL_SINGLEGAIN_MASK | _ADC_CAL_SINGLEOFFSET_MASK | _ADC_CAL_SINGLEOFFSETINV_MASK);
    ADC0->CAL |= (DEVINFO->ADC0CAL1 & 0x7FFF0000) >> 16; // Calibration for 5V reference
    ADC0->BIASPROG = (ADC0->BIASPROG & (_ADC_BIASPROG_VFAULTCLR_MASK | _ADC_BIASPROG_ADCBIASPROG_MASK)) | ADC_BIASPROG_GPBIASACC_HIGHACC;
    ...
    float fRXLOVPwr = ADC0->SINGLEDATA * 5000.f / 65535.f;

    return fRXLOVPwr * ADC_RX_LO_VPWR_DIV;
}
float adc_get_tx_lo_vpwr()
{
    while(ADC0->STATUS & (ADC_STATUS_WARM | ADC_STATUS_SCANREFWARM | ADC_STATUS_SINGLEREFWARM)); // Wait for ADC to cool down

    ADC0->SINGLECTRL = ADC_SINGLECTRL_AT_256CYCLES | ADC_SINGLECTRL_NEGSEL_VSS | ADC_TX_LO_VPWR_CHAN | ADC_SINGLECTRL_REF_5V | ADC_SINGLECTRL_RES_OVS;
    ADC0->SINGLECTRLX = ADC_SINGLECTRLX_FIFOOFACT_OVERWRITE | (0 << _ADC_SINGLECTRLX_DVL_SHIFT);
    ADC0->CAL &= ~(_ADC_CAL_SINGLEGAIN_MASK | _ADC_CAL_SINGLEOFFSET_MASK | _ADC_CAL_SINGLEOFFSETINV_MASK);
    ADC0->CAL |= (DEVINFO->ADC0CAL1 & 0x7FFF0000) >> 16; // Calibration for 5V reference
    ADC0->BIASPROG = (ADC0->BIASPROG & (_ADC_BIASPROG_VFAULTCLR_MASK | _ADC_BIASPROG_ADCBIASPROG_MASK)) | ADC_BIASPROG_GPBIASACC_HIGHACC;
    ...
    float fTXLOVPwr = ADC0->SINGLEDATA * 5000.f / 65535.f;

    return fTXLOVPwr * ADC_TX_LO_VPWR_DIV;
}