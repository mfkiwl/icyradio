...
static uint16_t adc_read_tx_pwr_vdet()
{
    ADC_REGS->ADC_INPUTCTRL = ADC_INPUTCTRL_GAIN_1X | ADC_INPUTCTRL_MUXNEG_GND | ADC_INPUTCTRL_MUXPOS(ADC_TX_PWR_VDET_DIV_CHAN);
...
static uint16_t adc_read_rx_pwr_vdet()
{
    ADC_REGS->ADC_INPUTCTRL = ADC_INPUTCTRL_GAIN_1X | ADC_INPUTCTRL_MUXNEG_GND | ADC_INPUTCTRL_MUXPOS(ADC_RX_PWR_VDET_DIV_CHAN);
    ADC_REGS->ADC_SAMPCTRL = ADC_SAMPCTRL_SAMPLEN(0x00);
...
uint32_t adc_get_tx_pwr_vdet()
{
    uint16_t usResult = adc_read_tx_pwr_vdet();

    return (uint64_t)usResult * 1000000UL * ADC_TX_PWR_VDET_DIV_RI / (ADC_TX_PWR_VDET_DIV_RF * 4095UL);
}
uint32_t adc_get_rx_pwr_vdet()
{
...
{
    uint16_t usResult = adc_read_tx_pwr_vdet();
    static const float fCorr = 1000000.f / 4095.f / ADC_TX_PWR_VDET_DIV_AV;
...
{
    uint16_t usResult = adc_read_rx_pwr_vdet();
    static const float fCorr = 1000000.f / 4095.f / ADC_RX_PWR_VDET_DIV;
