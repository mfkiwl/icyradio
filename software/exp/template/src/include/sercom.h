...
// #define SERCOM4_MODE_I2C_MASTER            // Define for Master I2C support
// #define SERCOM4_MODE_I2C_SLAVE             // Define for Slave I2C support
// #define SERCOM4_MODE_SPI                   // Define for SPI
// #define SERCOM4_MODE_UART                  // Define for UART
...
{
    return sercom4_i2c_master_transmit((ubAddress << 1) & ~0x01, pubSrc, ulCount, ubStop);
}
static inline uint8_t sercom4_i2c_master_read(uint8_t ubAddress, uint8_t *pubDst, uint32_t ulCount, uint8_t ubStop)
{
    return sercom4_i2c_master_transmit((ubAddress << 1) | 0x01, pubDst, ulCount, ubStop);
}
static inline uint8_t sercom4_i2c_master_write_byte(uint8_t ubAddress, uint8_t ubData, uint8_t ubStop)
{
    return sercom4_i2c_master_transmit((ubAddress << 1) & ~0x01, &ubData, 1, ubStop);
}
static inline uint8_t sercom4_i2c_master_read_byte(uint8_t ubAddress, uint8_t ubStop)
{
    uint8_t ubData;

    if(!sercom4_i2c_master_transmit((ubAddress << 1) | 0x01, &ubData, 1, ubStop))
        return 0;
...
static inline void sercom4_spi_transfer(const uint8_t *pubSrc, uint32_t ulSize, uint8_t *pubDst)
{
    if(!pubSrc)
        return;
    ...
    while(ulSize--)
        *pubDst++ = sercom4_spi_transfer_byte(*pubSrc++);
}
static inline void sercom4_spi_write(const uint8_t *pubSrc, uint32_t ulSize, const uint8_t ubWait)
{
    if(!pubSrc)
        return;

    while(ulSize--)
        sercom4_spi_write_byte(*pubSrc++, ubWait && !ulSize);
}
static inline void sercom4_spi_read(uint8_t *pubDst, uint32_t ulSize, uint8_t ubSendData)
{
    if(!pubDst)
        return;

    while(ulSize--)
        *pubDst++ = sercom4_spi_transfer_byte(ubSendData);
}
...
static inline void sercom4_uart_write(const uint8_t *pubSrc, uint32_t ulSize)
{
    while(ulSize--)
        sercom4_uart_write_byte(*pubSrc++);
}
static inline void sercom4_uart_read(uint8_t *pubDst, uint32_t ulSize)
{
    while(ulSize--)
        *pubDst++ = sercom4_uart_read_byte();
}
#endif // SERCOM4_MODE_UART


// #define SERCOM5_MODE_I2C_MASTER            // Define for Master I2C support
// #define SERCOM5_MODE_I2C_SLAVE             // Define for Slave I2C support
// #define SERCOM5_MODE_SPI                   // Define for SPI
// #define SERCOM5_MODE_UART                  // Define for UART
...
{
    return sercom5_i2c_master_transmit((ubAddress << 1) & ~0x01, pubSrc, ulCount, ubStop);
}
static inline uint8_t sercom5_i2c_master_read(uint8_t ubAddress, uint8_t *pubDst, uint32_t ulCount, uint8_t ubStop)
{
    return sercom5_i2c_master_transmit((ubAddress << 1) | 0x01, pubDst, ulCount, ubStop);
}
static inline uint8_t sercom5_i2c_master_write_byte(uint8_t ubAddress, uint8_t ubData, uint8_t ubStop)
{
    return sercom5_i2c_master_transmit((ubAddress << 1) & ~0x01, &ubData, 1, ubStop);
}
static inline uint8_t sercom5_i2c_master_read_byte(uint8_t ubAddress, uint8_t ubStop)
{
    uint8_t ubData;

    if(!sercom5_i2c_master_transmit((ubAddress << 1) | 0x01, &ubData, 1, ubStop))
        return 0;
...
static inline void sercom5_spi_transfer(const uint8_t *pubSrc, uint32_t ulSize, uint8_t *pubDst)
{
    if(!pubSrc)
        return;
    ...
    while(ulSize--)
        *pubDst++ = sercom5_spi_transfer_byte(*pubSrc++);
}
static inline void sercom5_spi_write(const uint8_t *pubSrc, uint32_t ulSize, const uint8_t ubWait)
{
    if(!pubSrc)
        return;

    while(ulSize--)
        sercom5_spi_write_byte(*pubSrc++, ubWait && !ulSize);
}
static inline void sercom5_spi_read(uint8_t *pubDst, uint32_t ulSize, uint8_t ubSendData)
{
    if(!pubDst)
        return;

    while(ulSize--)
        *pubDst++ = sercom5_spi_transfer_byte(ubSendData);
}
...
static inline void sercom5_uart_write(const uint8_t *pubSrc, uint32_t ulSize)
{
    while(ulSize--)
        sercom5_uart_write_byte(*pubSrc++);
}
static inline void sercom5_uart_read(uint8_t *pubDst, uint32_t ulSize)
{
    while(ulSize--)
        *pubDst++ = sercom5_uart_read_byte();
}
